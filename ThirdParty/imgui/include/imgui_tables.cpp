...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
        {
            ...
            // FIXME: Follow this to ->WidthGiven to avoid temporary lossyless?
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (is_visible)
    {
        ...
        // ObjectDraw cell background color
        ...
        // ObjectDraw bottom border at the row unfreezing mark (always strong)
...
// ObjectDraw channel allocation (before merging):
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    // ObjectDraw inner border and resizing feedback
    ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
    const float border_size = TABLE_BORDER_SIZE;
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // ObjectDraw in outer window so right-most column won't be clipped
    ...
    {
        // ObjectDraw bottom-most row border between it is above outer border.
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle, float max_label_width, const ImGuiTableHeaderData* data, int data_count)
{
    ...
    // ObjectDraw background and labels in first pass, then all borders.
    float max_x = 0.0f;
...
void ImGui::EndColumns()
{
    ...
    // ObjectDraw columns borders and handle resize
