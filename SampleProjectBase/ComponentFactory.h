...
namespace HashiTaku
{
	...
	public:
		...
		template<ComponentConcept T> std::unique_ptr<T> Create();
	...
	private:
		...
		template<ComponentConcept T> void ResistComponnent();

		template<ComponentConcept T> static std::string GetComponentName();
	};
	...
	inline std::unique_ptr<T> ComponentFactory::Create()
	{
		...
		// ダウンキャストする
		std::unique_ptr<T> createComp =
			std::unique_ptr<T>(static_cast<T*>(createBaseComp.release()));
	...
	inline std::string ComponentFactory::GetComponentName()
	{
		...
		constexpr u_int length = 20;

		if (componentName.find("class HashiTaku::CP_") == 0) {
			// 'CP_' を取り除く
