...
namespace HashiTaku
{
    ...
    public:
        ...
        /// @return アセットのポインタ
        template<AssetConcept T> static T* SetAsset(const std::string& _assetName, std::unique_ptr<T> _pAsset);

        /// @brief アセットがあるか確認
        /// @tparam T アセットの型
        /// @param _assetName 名前
        /// @return あるフラグ
        template<AssetConcept T> static bool CheckImport(const std::string& _assetName);
