...
namespace HashiTaku
{
	...
	void BossActionController::Load(const json& _data)
	{
		...
		bool isLoad = LoadJsonEnum<BossActState_Base::BossState>(
			"defaultState",
			defaultState,
			_data);
	...
	BossActState_Base::BossState BossActionController::RandState(ActDistance _actDistance)
	{
		...
		// 確率でステートを選ぶ
		float rand = Random::Range<float>(0.0f, 1.0f);
	...
	BossActState_Base* BossActionController::CastBossAct(StateNode_Base* _pBaseNode)
	{
