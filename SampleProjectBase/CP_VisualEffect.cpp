...
namespace HashiTaku
{
	...
	void CP_VisualEffect::Load(const nlohmann::json& _data)
	{
		...
		LoadJsonBoolean("isTrimming", isTrimming, _data);
		if (isTrimming)
		{
			LoadJsonInteger("startFrame", playStartFrame, _data);
			LoadJsonInteger("endFrame", playEndFrame, _data);
		}

		std::string vfxName;
		if (LoadJsonString("vfxName", vfxName, _data))
		{
