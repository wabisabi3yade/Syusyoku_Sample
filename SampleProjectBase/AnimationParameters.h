...
namespace HashiTaku
{
	...
	class AnimationParameters : public IImGuiUser, public ISaveLoad
	{
		/// @brief パラメータリスト
		std::unordered_map<std::string, AnimParam::conditionValType> animParameters;

		/// @brief トリガーの状態をリセットための配列
		std::list<TriggerType*> resetTriggers;

		/// @brief パラメータ削除時の通知サブジェクト
		std::unique_ptr<Subject<AnimParam::NotificationData>> pAnimParamSubject;
	public:
		AnimationParameters();
		~AnimationParameters() {}
		...
		const AnimParam::conditionValType* GetValueAddress(const std::string& _name) const;
		...
		/// @return パラメータの型
		AnimParam::TypeKind GetParameterType(const std::string& _parameterName) const;
		...
		/// @return サブジェクトの参照
		Subject<AnimParam::NotificationData>& GetParamRemoveSubject();
	...
	private:
		...
		/// @return 判別タイプ
		AnimParam::TypeKind GetType(const AnimParam::conditionValType& _parameter);
	...
	inline void AnimationParameters::AddParameter(std::string _paramNames)
	{
		...
		AnimParam::conditionValType addParam;
