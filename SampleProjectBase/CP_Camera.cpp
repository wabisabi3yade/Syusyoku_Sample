...
namespace HashiTaku
{
	...
	void CP_Camera::UpdateViewMatrix()
	{
		...
		// ビュー変換行列を求める
		DXSimp::Matrix viewMatrix = DirectX::XMMatrixLookAtLH
		(
			GetTransform().GetPosition(),		// カメラ座標
			target,	// 注視点
			DXSimp::Vector3::Up // 上ベクトル
		);
	...
	void CP_Camera::UpdatePerspective()
	{
		...
		// ビュー変換行列を作成する
		DXSimp::Matrix mat = DirectX::XMMatrixPerspectiveFovLH(
			fov * Mathf::degToRad,
	...
	void CP_Camera::UpdateOrthographic()
	{
		...
		// 正投影行列を作成する
		DXSimp::Matrix mat = DirectX::XMMatrixOrthographicOffCenterLH(
			0.0f,	// 左上
			screenWidth, 		// 右上
			screenHeight,		// 左下
			0.0f,	// 右下
