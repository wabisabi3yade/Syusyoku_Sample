...
void AnimationChannel::AddPosKey(float _startTime, const DirectX::SimpleMath::Vector3& _position)
{
	AnimKey_V3 addPosKey{ _position, _startTime };
	positionKeys.push_back(addPosKey);
}

void AnimationChannel::AddScaleKey(float _startTime, const DirectX::SimpleMath::Vector3& _scale)
{
	AnimKey_V3 addScaleKey{ _scale, _startTime };
	scaleKeys.push_back(addScaleKey);
}

void AnimationChannel::AddQuatKey(float _startTime, const DirectX::SimpleMath::Quaternion& _quaternion)
{
	AnimKey_Q addQuatKey{ _quaternion, _startTime };
	quaternionKeys.push_back(addQuatKey);
}

u_int AnimationChannel::FindPrevPosKey(float _playingTime) const
{
	...
	{
		if (_playingTime < positionKeys[k_i + 1].startTime)
		{
			prevKeyNum = k_i;
			break;
		}
...
u_int AnimationChannel::FindPrevScaleKey(float _playingTime) const
{
	...
	{
		if (_playingTime < scaleKeys[k_i + 1].startTime)
		{
			prevKeyNum = k_i;
			break;
		}
...
u_int AnimationChannel::FindPrevQuatKey(float _playingTime) const
{
	...
	{
		if (_playingTime < quaternionKeys[k_i + 1].startTime)
		{
			prevKeyNum = k_i;
			break;
		}
