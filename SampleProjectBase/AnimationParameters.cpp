...
namespace HashiTaku
{
	...
	{
		pAnimParamSubject = std::make_unique<Subject<NotificationData>>();
	}
	...
	AnimParam::TypeKind AnimationParameters::GetParameterType(const std::string& _parameterName) const
	{
		// パラメータリストにないなら
	...
	Subject<AnimParam::NotificationData>& AnimationParameters::GetParamRemoveSubject()
	{
	...
	void AnimationParameters::Load(const json& _data)
	{
		for (auto& paramData : _data)
		{
			// パラメータ名
			std::string name;
			if (!LoadJsonString("name", name, paramData)) continue;

			// パラメータ型
			TypeKind type;
			if (!LoadJsonEnum<TypeKind>("type", type, paramData)) continue;
			...
			case TypeKind::Bool:
			{
				bool isBool = false; LoadJsonBoolean(element, isBool, paramData);
				value = isBool;
			}
			break;
			...
			case TypeKind::Int:
			{
				int intVal = 0;  LoadJsonInteger(element, intVal, paramData);
				value = intVal;
			}
			break;
			...
			case TypeKind::Float:
			{
				float floatVal = 0.0f; LoadJsonFloat(element, floatVal, paramData);
				value = floatVal;
			}
			break;
