...
void CP_BattleManager::Load(const nlohmann::json& _data)
{
	...
	HashiTaku::LoadJsonString("fade", fadeObjName, _data);
	HashiTaku::LoadJsonString("start", startObjName, _data);
	HashiTaku::LoadJsonString("win", winObjName, _data);
	HashiTaku::LoadJsonString("gameOver", gameOverObjName, _data);

	nlohmann::json loadData;
	if (HashiTaku::LoadJsonData("gameStartCurve", loadData, _data))
	{
		gameStartCurve.Load(loadData);
	}

#ifdef EDIT
	HashiTaku::LoadJsonBoolean("uiAnim", isUIAnimation, _data);
#endif // EDIT
}
