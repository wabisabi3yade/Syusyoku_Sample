...
namespace HashiTaku
{
	...
	void PlayerRushAttack::Load(const json& _data)
	{
		...
		LoadJsonFloat("slashBegin", slashBeginDistance, _data);
		LoadJsonFloat("horiDis", progressLengthHori, _data);
		LoadJsonFloat("cancelAnim", cancelBeginRatio, _data);
		LoadJsonFloat("slashAnim", slashBeginAnimRatio, _data);
		if (IsJsonContains(_data, "horiCurve"))
		{
