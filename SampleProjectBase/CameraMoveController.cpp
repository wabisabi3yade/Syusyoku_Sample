...
namespace HashiTaku
{
	...
	void CameraMoveController::Load(const nlohmann::json& _data)
	{
		...
		if (LoadJsonDataArray("shakeLevels", shakeDatas, _data))
		{
			u_int levelId = 0;
			for (auto& shData : shakeDatas)
			{
				...
				LoadJsonFloat("duration", shLvParam.duration, shData);
				LoadJsonFloat("power", shLvParam.power, shData);
				levelId++;
			}
		}
	}
	...
	DirectX::SimpleMath::Vector3 CameraMoveController::CalcShakeOffset(ShakeLevel _level, ShakeType _type)
	{
		...
		case ShakeType::Vertical:
			offset.y = Random::Range<float>(-1.0f * param.power, param.power);
			break;

		case ShakeType::Horizon:
			offset.x = Random::Range<float>(-1.0f * param.power, param.power);
			break;
